...
window.Vaadin['_vaadintheme_todo_globalCss'] = window.Vaadin['_vaadintheme_todo_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_todo_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_todo_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_todo_globalCss'].push(target);
  }
  if (!document['_vaadintheme_todo_componentCss']) {
    
    document['_vaadintheme_todo_componentCss'] = true;
  }
